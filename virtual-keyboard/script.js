  ...
  this.keys.forEach((a, i) => {
    ...
    if (element === event.code) {
      if (event.code === 'Space') {
        ...
        a.classList.add('active');
        a.classList.add('anime');
        if (lang === false) { lang = true; } else { lang = false; }
...
/* eslint-disable no-undef */
  if (localStorage.getItem('lang') === 'true') {
    ...
    keys.forEach((val, index) => {
      const whatKey = val;
      whatKey.innerHTML = keysEng[index];
    });
  }
  if (localStorage.getItem('lang') === 'false') {
    keys.forEach((val, index) => {
      const whatKey = val;
      whatKey.innerHTML = keysLayout[index];
    });
  }
/* eslint-enable no-undef */
}
init();
createKeys();
document.addEventListener('DOMContentLoaded', isLang);
